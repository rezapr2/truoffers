...
Redaction keeps the record (URL, page title, check date) but deletes the excerpt text. The
retention job runs nightly at 03:00 (`RetentionService`).
...
4. Listings created from that website that no business has claimed are hidden (`suspended`) until
   an administrator reviews them.
5. Administrators see the request in the unacknowledged opt-out queue.
